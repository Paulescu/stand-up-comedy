...
class CheckpointEveryNSteps(pl.Callback):
    ...
    def on_train_batch_end(self, trainer: pl.Trainer, _):
        ...
        if global_step % self.save_step_frequency == 0:
            ...
            # delete previous checkpoint of this kind to avoid cluttering disk
            # https://github.com/PyTorchLightning/pytorch-lightning/issues/1366
            # stop()
            if os.path.isfile(self.prev_ckpt_path):
                os.remove(self.prev_ckpt_path)
...
def cli_main():
    ...
    checkpoint_every_n_steps = CheckpointEveryNSteps(save_step_frequency=100)

    logger = TensorBoardLogger(args.tensorboard_dir, name='gpt2comedian')

    trainer = pl.Trainer.from_argparse_args(
        args,
        callbacks=[checkpoint_callback, checkpoint_every_n_steps],
        logger=logger
    )
